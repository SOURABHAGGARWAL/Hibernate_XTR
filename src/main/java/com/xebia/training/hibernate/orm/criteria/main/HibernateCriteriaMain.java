package com.xebia.training.hibernate.orm.criteria.main;
...
import com.xebia.training.hibernate.orm.l2_cache.model.Employee;
import com.xebia.training.hibernate.orm.l2_cache.util.HibernateUtil;
...
public class HibernateCriteriaMain {
	...
	public static void main(String[] args) {
		...
		for (Employee emp : empList) {
			System.out.println("ID=" + emp.getId() + ", Zipcode="
					+ emp.getAddress().getZipcode());
		}

		// Get with ID, creating new Criteria to remove all the settings
		criteria = session.createCriteria(Employee.class).add(
				Restrictions.eq("id", new Long(3)));
		...
		empList = session.createCriteria(Employee.class)
				.addOrder(Order.desc("id")).setFirstResult(0).setMaxResults(2)
				.list();
		...
		for (Employee emp4 : empList) {
			System.out.println("Employees having 'i' in name::"
					+ emp4.getName() + "," + emp4.getAddress().getCity());
		}
